...
function loadStoryboardWorkspace(draft: CarouselDraft): StoredStoryboardWorkspace & { recoveredVariants: boolean } {
  ...
  try {
    ...
    const active = Math.max(0, Math.min(2, Number(stored.active) || 0));
    ...
    const recoveredVariants = localIsNewer && Array.isArray(stored.variants) && stored.variants.length === 3;
    return {
...
export function App() {
  ...
  const [workspaceSection, setWorkspaceSection] = useState<"research" | "carousels">("research");
  ...
  const [activeDraft, setActiveDraft] = useState<CarouselDraft | null>(null);
  ...
  useEffect(() => {
    void Promise.all([api.health(), refreshSessions(), refreshCarouselProjects()])
  ...
  }, [refreshCarouselProjects, refreshSessions]);
    ...
    <div className="app-shell">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <button className="button button-primary" onClick={() => setNewResearchOpen(true)}>
      ...
      <div className="workspace">
        ...
        <main className="main-stage">
          ...
          {loading ? <LoadingState /> : workspaceSection === "carousels" ? (
      ...
      {activePost && <PostModal post={activePost} onClose={() => setActivePost(null)} onStatus={handleStatus} onPin={handlePin} />}
      ...
      {activeDraft && <StoryboardModal draft={activeDraft} onClose={() => { setActiveDraft(null); void refreshCarouselProjects(); if (session?.id === activeDraft.sessionId) void refreshSession(activeDraft.sessionId, true); }} />}
    </div>
  );
}
...
function NewRunModal({ project, onClose, onCreate }: { project: ResearchProject; onClose: () => void; onCreate: (input: { title?: string; queries: string[]; targetResults: number; excludeSeen: boolean; saveAsDefaults: boolean }) => Promise<void> }) {
  ...
  const recommendedQueries = Math.max(3, Math.ceil(targetResults / 140));
  const perQueryLimit = Math.min(250, Math.max(20, Math.ceil((targetResults / Math.max(1, queries.length)) * 1.45)));
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !creating) onClose(); }}>
      <section className="run-modal" role="dialog" aria-modal="true" aria-label={`Новый проход: ${project.title}`}>
        ...
        <div className="run-form">
          ...
          <fieldset className="target-picker"><legend>Сколько уникальных каруселей собрать</legend><div>{[100, 300, 500, 1_000].map((value) => <button key={value} className={targetResults === value ? "is-active" : ""} onClick={() => setTargetResults(value)}>{value.toLocaleString("ru")}</button>)}<label><input type="number" min="25" max="1000" step="25" value={targetResults} onChange={(event) => setTargetResults(Math.max(25, Math.min(1_000, Number(event.target.value) || 25)))} /><span>своя цель</span></label></div></fieldset>
    ...
    <>
      ...
      <section className="research-meter" aria-label="Прогресс исследования">
        ...
        <div className="meter-progress">
          <span><strong>{aiRunning ? `AI ${aiProgress}%` : session.searchRunning ? `${progress}%` : session.status === "complete" ? "Готово" : "Не запущено"}</strong><small>{aiRunning ? `${session.aiJob?.completed || 0} из ${session.aiJob?.total || 0} · только текст` : session.currentQuery || "поиск TikTok"}</small></span>
          <i><b style={{ width: `${aiRunning ? aiProgress : session.status === "complete" ? 100 : progress}%` }} /></i>
        </div>
      </section>
      ...
      {!session.posts.length ? (
        ...
        <div className="bucket-stack">
          {(["skip", "maybe", "relevant", "pending", "low_traction"] as DisplayBucket[]).map((status) => (
            <Bucket key={status} status={status} posts={buckets[status]} onOpenPost={onOpenPost} onStatus={onStatus} onPin={onPin} />
          ))}
        </div>
      )}
      </>}
    ...
    <section className={`bucket bucket-${status}`}>
      ...
      {!collapsed && (
        <div className="post-grid">
          {posts.map((post) => <PostCard key={post.id} post={post} onOpen={() => onOpenPost(post)} onStatus={onStatus} onPin={onPin} />)}
        </div>
      )}
    </section>
  );
...
function PostCard({ post, onOpen, onStatus, onPin }: { post: SessionPost; onOpen: () => void; onStatus: (post: SessionPost, status: DecisionStatus | null) => void; onPin: (post: SessionPost, pinned: boolean) => void }) {
...
function PostModal({ post, onClose, onStatus, onPin }: { post: SessionPost; onClose: () => void; onStatus: (post: SessionPost, status: DecisionStatus | null) => void; onPin: (post: SessionPost, pinned: boolean) => void }) {
  useEffect(() => {
...
function StoryboardModal({ draft, onClose }: { draft: CarouselDraft; onClose: () => void }) {
  ...
  const updateSlide = (update: Partial<typeof slide>) => {
    setSaveState("local");
    setVariants((current) => current.map((item, variantIndex) => variantIndex !== active ? item : {
      ...item,
  ...
  const persistVariants = useCallback((nextVariants: StoryboardVariant[]) => {
    ...
    setSaving(true);
    setSaveState("saving");
    const queued = saveQueue.current.catch(() => undefined).then(() => api.saveDraft(draft.sessionId, draft.id, nextVariants));
  ...
  const exportZip = async (includeText: boolean) => {
    ...
    try {
      ...
      const blob = await api.exportDraft(draft.sessionId, draft.id, active, includeText);
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const created = await api.createSession({
        ...
        targetResults: Math.max(25, Math.min(1_000, Number(form.targetResults) || 100)),
      });
      onCreated(created);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : String(requestError));
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
      <form className="research-modal" onSubmit={submit}>
        ...
        <div className="form-grid">
          ...
          <label className="field"><span>Первый тестовый проход</span><input type="number" min="25" max="1000" step="25" value={form.targetResults} onChange={(event) => update("targetResults", event.target.value)} /><small>Для новой ниши разумно начать с 50–100 результатов.</small></label>
